...
class LogisticsOptimizer:
    ...
    def generate_sample_data(self, n_points=150):
        ...
        for i, (lat_center, lon_center, n_stores) in enumerate(concentrations):
            for _ in range(n_stores):
                ...
                volume = max(1, int(np.random.lognormal(3, 1)))
    ...
    def load_data(self, df=None, filepath=None):
        ...
        # Limpeza básica
        self.df = self.df.dropna(subset=['LATITUDE', 'LONGITUDE', 'VOLUME_PICKUP'])
        self.df = self.df[self.df['VOLUME_PICKUP'] > 0]

        if len(self.df) == 0:
            raise ValueError(
                "Nenhum dado válido encontrado após limpeza. "
                "Verifique as colunas LATITUDE, LONGITUDE e VOLUME_PICKUP"
            )
        ...
        if invalid_coords.sum() > 0:
            print(f"Removendo {invalid_coords.sum()} pontos com coordenadas inválidas")
        ...
        print(f"Volume - Min: {self.df['VOLUME_PICKUP'].min()}, Max: {self.df['VOLUME_PICKUP'].max()}")
    ...
    def exploratory_analysis(self):
        ...
        print(f"Volume total de pacotes: {self.df['VOLUME_PICKUP'].sum()}")
        print(f"Volume médio por ponto: {self.df['VOLUME_PICKUP'].mean():.1f}")
        print(f"Volume mediano: {self.df['VOLUME_PICKUP'].median():.1f}")
        print(f"Desvio padrão do volume: {self.df['VOLUME_PICKUP'].std():.1f}")
        ...
        return {
            ...
            'total_volume': self.df['VOLUME_PICKUP'].sum(),
            'avg_volume': self.df['VOLUME_PICKUP'].mean(),
    ...
    def calculate_distance_matrix(self, sample_size=None):
        ...
        df_sample = self.df.sample(n=min(sample_size or len(self.df), 200))
    ...
    def find_optimal_clusters(self, max_clusters=12):
        ...
        weights = self.df['VOLUME_PICKUP'].values

        scaler = StandardScaler()
        X_scaled = scaler.fit_transform(X)

        weights_scaled = (weights - weights.min()) / (weights.max() - weights.min())
        X_weighted = np.column_stack([X_scaled, weights_scaled * 0.5])
    ...
    def perform_clustering(self, n_clusters=None):
        """Executa clustering geográfico ponderado por volume"""
        ...
        for cluster_id in range(n_clusters):
            ...
            stats = {
                ...
                'total_volume': cluster_data['VOLUME_PICKUP'].sum(),
                'avg_volume_per_point': cluster_data['VOLUME_PICKUP'].mean(),
    ...
    def monte_carlo_simulation(self, n_simulations=5000):
        ...
        max_reasonable_time = working_hours_per_day * 0.8
            ...
            for capacity in vehicle_capacities:
                for trips in trips_per_day:
                    ...
                    for sim in range(n_simulations):
                        ...
                        time_variation = np.random.uniform(0.9, 1.1)
    ...
    def generate_fleet_recommendations(self):
        """Gera recomendações finais de frota por cluster e cenário"""
    ...
    def create_interactive_map(self):
        ...
        for cluster_id in sorted(self.df['CLUSTER'].unique()):
            ...
            for _, row in cluster_data.iterrows():
                folium.CircleMarker(
                    ...
                    radius=max(3, min(15, row['VOLUME_PICKUP'] / 10)),
                    popup=f"""
                    <b>Cluster {cluster_id}</b><br>
                    Seller ID: {row['CLIENTEseller_id']}<br>
                    Volume: {row['VOLUME_PICKUP']} pacotes<br>
                    Cidade: {row['CITY']}<br>
        ...
        for _, cluster in self.clusters.iterrows():
            ...
            folium.Circle(
                location=[cluster['centroid_lat'], cluster['centroid_lon']],
                radius=cluster['max_radius_km'] * 1000,
    ...
    def run_complete_analysis(self, df=None, n_clusters=None):
        """Executa análise completa end-to-end"""
    ...
    def export_clustered_data(self, filename=None, add_timestamp=True):
        """Exporta o DataFrame com clusters para Excel"""
        if self.df is None or 'CLUSTER' not in self.df.columns:
            raise ValueError("Execute o clustering primeiro!")
        ...
        export_df = export_df.sort_values(['CLUSTER', 'VOLUME_PICKUP'], ascending=[True, False])

        with pd.ExcelWriter(filename, engine='openpyxl') as writer:
            export_df.to_excel(writer, sheet_name='Dados_com_Clusters', index=False)

            if self.clusters is not None:
                self.clusters.copy().to_excel(writer, sheet_name='Resumo_Clusters', index=False)
            ...
            for cluster_id in sorted(export_df['CLUSTER'].unique()):
                ...
                stats_data.append({
                    ...
                    'Total_Volume': cluster_data['VOLUME_PICKUP'].sum(),
                    'Volume_Medio': cluster_data['VOLUME_PICKUP'].mean(),
                    'Principais_Cidades': ', '.join(
                        cluster_data['CITY'].value_counts().head(3).index.tolist()
                    )
                })

            pd.DataFrame(stats_data).to_excel(writer, sheet_name='Estatisticas_Clusters', index=False)
    ...
    def export_fleet_recommendations(self, filename=None, add_timestamp=True):
        ...
        with pd.ExcelWriter(filename, engine='openpyxl') as writer:
            ...
            self.simulation_results.copy().to_excel(writer, sheet_name='Simulacao_Detalhada', index=False)
            ...
            pd.DataFrame(cluster_analysis).to_excel(writer, sheet_name='Analise_por_Cluster', index=False)
    ...
    def export_interactive_map(self, filename=None, add_timestamp=True, add_info_panel=True):
        ...
        if add_info_panel:
            ...
            <p><b>Clusters:</b> {n_clusters}</p>
            <p><b>Pickup Points:</b> {total_points}</p>
            <p><b>Total Volume:</b> {total_volume:,} packages</p>
            <p><b>Avg per Point:</b> {total_volume/total_points:.1f} packages</p>
            <p><b>Analysis Date:</b> {datetime.now().strftime('%Y-%m-%d')}</p>
            <hr style="border: 1px solid #ddd;">
            <p style="font-size: 10px; color: #666;">
                Circle size = pickup volume<br>
                Markers = cluster centroids<br>
                Rings = coverage radius
            </p>
            </div>
            '''
